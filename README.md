...
An enterprise-grade, multi-tier web application and observability platform deployed on AWS using Terraform (IaC), Ansible, Docker Compose, PyMySQL/Python, and an automated GitHub Actions CI/CD pipeline.

---

## What NeuroGrid Is All About

**NeuroGrid** is a distributed neural compatibility and implant assessment platform engineered with zero-trust network isolation, auto-scaling compute, and telemetry.
...
* **Real-Time Dynamic Telemetry:** Exposes runtime metrics (`/metrics`) using `prometheus-flask-exporter` alongside host hardware metrics from `node_exporter`.

---

## Architecture Overview

![Architecture Diagram](architecture/photo.png) 
...
* **Dynamic API Reverse Proxy:** CloudFront routes `/api/*` traffic to an Application Load Balancer (ALB) across multiple public availability zones.
...
* **Managed Multi-AZ Database:** Amazon RDS MySQL 8.0 configured with synchronous multi-AZ standby replication across private database subnets, strictly accessible from backend compute security groups over port 3306.
* **Dedicated Standalone Observability Host:** Standalone EC2 instance in a public subnet running Prometheus and Grafana in Docker. Ingress ports 3000 and 9090 are closed to the public internet. It uses AWS EC2 Service Discovery (`ec2_sd_configs`) via an IAM Role to scrape dynamic ASG worker nodes over private VPC IPs.
* **Zero-Trust Management (No Port 22 / Open SSH):** Inbound SSH from the internet is disabled across the entire infrastructure. All instance provisioning, Ansible deployment orchestration, and administrative dashboard access execute through AWS Systems Manager (SSM) Session Manager.

---

## End-to-End User Flow & Telemetry Lifecycle

1. **Edge Resolution & Static Delivery:** When a user visits `https://<CLOUDFRONT_DOMAIN>`, Route 53 directs the request to the nearest CloudFront edge location. CloudFront matches the default root pattern (`/*`) and fetches the static web assets from the private Amazon S3 bucket via Origin Access Control (OAC). S3 denies direct public read requests; it only authorizes CloudFront's signed OAC principal.
2. **Dynamic API Routing & SSL Termination:** When the user signs up, logs in, or submits neural bio-telemetry, the client executes an asynchronous `POST` request to `/api/*`. CloudFront identifies the `/api/*` path rule, disables edge caching, forwards client origin headers via `ProxyFix(x_for=2)`, and routes the HTTPS payload to the internet-facing Application Load Balancer (ALB).
3. **Private Subnet Load Distribution:** The ALB accepts the request and distributes it over internal port 80 to healthy EC2 instances inside private application subnets across AZ-a and AZ-b. These instances accept connections exclusively originating from the ALB Security Group.
4. **Algorithmic Processing & Multi-AZ Database Writes:** The containerized Flask backend processes the data against its scoring matrix. It establishes a pooled, thread-safe connection using `PyMySQL` and `DBUtils` over private port 3306 to Amazon RDS MySQL. RDS commits the user record and synchronously mirrors the transaction across to its standby replica in the secondary AZ before returning a 200 JSON payload back through the ALB and CloudFront to the user's browser.
5. **Decoupled Telemetry Scraping:** Every HTTP request triggers `prometheus-flask-exporter` instrumentation to update metric counters and histograms. Concurrently, the dedicated monitoring EC2 instance queries the AWS EC2 API using attached IAM role permissions to discover active backend nodes. Prometheus scrapes application metrics on port 80 (`/metrics`) and hardware performance metrics on port 9100 (`node_exporter`).
6. **Encrypted Administration:** Operators access internal dashboards without opening firewall rules. An SSM port-forwarding session opens a TLS-encrypted tunnel directly to the monitoring host, binding Grafana (`localhost:3000`) and Prometheus (`localhost:9090`) to the local machine.
...
```bash
git add .
git commit -m "Deploy complete production infrastructure, Ansible deployment pipeline, and monitoring stack"
...
4. **ASG Backend Deployment via Ansible (SSM SSH Tunnel):**
* Dynamically discovers all active `InService` instances in the Auto Scaling Group.
* Injects an ephemeral SSH key into instances via AWS Systems Manager (`AWS-RunShellScript`).
* Opens an encrypted SSH proxy tunnel over SSM (`AWS-StartSSHSession`).
* Executes `ansible/deploy-backend.yml`, automatically pulling the verified RDS endpoint via AWS CLI fallback.
* Injects environment variables into `/home/ubuntu/app/.env` and deploys `neurogrid_backend` and `node_exporter` via Docker Compose.


5. **Monitoring Deployment via Ansible:**
* Dispatches `ansible/deploy-monitoring.yml` to the dedicated monitoring instance (`tag:Role=monitoring`).
* Starts Prometheus and Grafana, immediately discovering and scraping ASG backend targets over the private VPC network.
